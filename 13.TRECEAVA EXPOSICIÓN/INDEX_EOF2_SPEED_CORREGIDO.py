# -*- coding: utf-8 -*-
"""
Created on Sat Nov 19 10:11:02 2016
...
up1    = np.where(latPR == 33)[0][0]
down1  = np.where(latPR == 22)[0][0]
left1  = np.where(lonPR == -80)[0][0]
right1 = np.where(lonPR == -68)[0][0]

up2    = np.where(latPR == 29.5)[0][0]
down2  = np.where(latPR == 20)[0][0]
left2  = np.where(lonPR == -97)[0][0]
right2 = np.where(lonPR == -90)[0][0]

INDICE = pd.DataFrame(index=fechas, columns=['serie1', 'serie2', 'indice', 'anom_index'])
for i, j in enumerate(INDICE.index):
    ...
    serie2 = PR.variables['msl'][i, up2:down2+1, left2:right2+1]/100
    serie2 = serie2*mask[up2:down2+1, left2:right2+1]
    SERIE2 = np.nanmean(serie2)    
     
    INDICE['serie1'][j] = SERIE1
    INDICE['serie2'][j] = SERIE2
    INDICE['indice'][j] = SERIE1-SERIE2

#==============================================================================
'''Sacando ciclo anual de los índices '''
#==============================================================================

ciclo =np.zeros(12)

for i in range(12):
    ciclo[i] = np.mean(INDICE.indice[(INDICE.index.month == i+1)])

#==============================================================================
'''Sacando anomalias de los índices '''
#==============================================================================

for i in range(12):
    INDICE['anom_index'][(INDICE.index.month == i+1)] = INDICE.indice[(INDICE.index.month == i+1)]-ciclo[i]
...
u   = UyV.variables['u10'][:len(fechas)]
v   = UyV.variables['v10'][:len(fechas)]
speed = np.sqrt(u*u+v*v)

#==============================================================================
'''Ciclo anual de velocidad del viento '''
#==============================================================================

ciclo_spd = np.zeros((12, u.shape[1], u.shape[2]))

for i in range(12):
    ciclo_spd[i] = np.mean(speed[(fechas.month == i+1)], axis =0) 

#==============================================================================
'''Anomalías de velocidad del viento '''
#==============================================================================

anom_spd = np.zeros(u.shape)

for i in range(12):
    anom_spd[(fechas.month == i+1)] = speed[(fechas.month == i+1)]-ciclo_spd[i]

#==============================================================================
'''MAPAS DE CORRELACIONES'''
#==============================================================================

def mapa_corr(anom_speed, serie, fechas):
    mapa = anom_speed[0, :, :] # la idea en ésta línea es captar la forma que tendrá el mapa final de correlaciones. Por eso es indistinto el uso de u10 o v10
...
corr_spd = mapa_corr(anom_spd, INDICE.anom_index, fechas)
...
def plot_mapa(mapa, variable, LAT, LON):
    ...
    cb = plt.colorbar(CF, orientation='horizontal', pad=0.05, shrink=0.8, boundaries=bounds, format='%.2f')
    ax.set_title('Field Correlations (INDEX'+'-'+variable+')', size='15')    
    map.fillcontinents(color='white')
    plt.show()

plot_mapa(corr_spd, 'Wind Speed Anomaly', Lat, Lon)
